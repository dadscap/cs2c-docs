...
description: Manage named portfolios, track transactions, import inventories, and value holdings.
order: 13
---

The Portfolio API supports both stateless valuation of ad-hoc item lists and stateful management of named portfolios.

Portfolio item capacity is tier-based:

- `free`: up to `1000` items per portfolio
- `pro`: up to `5000` items per portfolio
- `quant`: up to `10000` items per portfolio

Manual item adds, Steam imports, and CSV imports all enforce the same per-portfolio cap.
...
### List Portfolios
>
> Returns all named portfolios for the authenticated user.
...
### Create Portfolio
>
> Creates a new empty portfolio. The maximum number of portfolios is tier-controlled.
...
### Delete Portfolio
>
> Deletes a portfolio and all associated items and transactions.
...
### List Items
>
> Lists all items currently stored in the portfolio.
...
### Add Item
>
> Manually adds a single item to a portfolio by `item_id` or `market_hash_name`.
...
- Use this path for manual entry when the item did not come from Steam import.

---

### Remove Item
>
> Removes a specific stored portfolio item entry.
...
- Items already present by Steam asset ID are skipped.
- Unmatched items are returned in `unresolved` and are not stored.
...
>
> Values all items in the saved portfolio at current market prices.
...
- This is live valuation against current indexed prices, not historical replay.

---

### Historical Saved Portfolio Valuation
>
> Replays holdings from the portfolio transaction ledger and returns daily historical valuation points.
...
| lookback | string | Optional day window like `30d`. Overrides `start_date`. |
| currency | string | Target currency. Default: `USD`. |
| providers | string[] | Optional provider filter. Repeat to pass multiple values. |
| limit | integer | Maximum daily points to return. |
| cursor | string | Opaque cursor from the previous page. |

- Daily only: transactions are stored as calendar dates, not timestamps.
- Holdings come from the ledger only, not from current saved portfolio items.
- Per-day valuation uses the best provider close after FX conversion, with provider carry-forward.
...
### List Transactions
>
> Returns the buy/sell ledger for the portfolio, newest first.
...
- `item_id` or `market_hash_name` must be supplied.
- Buy transactions can create portfolio items for untracked items.
...
- Format is auto-detected from the uploaded CSV headers.
- Buy transactions in CSV import can create missing portfolio items automatically.
